...
DailyUsageEngine.prototype.generateUsages = function () {
    ...
    this.simulator.airvantage.importUsage(data).then((result) => {
        console.log(`Operation '${result.operation}' status:`);
        self.simulator.airvantage.queryOperations({ uid: result.operation }).then((result) => console.log(result));
    });
};

DailyUsageEngine.prototype.start = function (systems) {
    this.subscriptions = _.map(systems, 'subscription');
    this.generateUsages();
};
