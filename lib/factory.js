...
let airvantage, simulation, applicationUid = "";

function checkApplication() {
    if (hasApplication()) {
        return airvantage.queryApplications({ name: getApplicationName(), labels: getSimulationLabel() })
...
function createFleet() {
    ...
    return Bromise.all(systemsCreation).then(function() {
        conzole.done("All systems were created");
    });
}
...
function getCleaningLabel() {
    ...
    if (simulation.labels && simulation.simulationLabel) {
        return simulation.simulationLabel;
    }

    throw new Error('Missing label or labels + cleaningLabel option to clean the simulation resources');
}

function cleanResources() {
    conzole.start("Clean resources with label: ", getCleaningLabel());
    return airvantage.deleteSystems({
            selection: { label: getCleaningLabel() },
            deleteGateways: true,
        ...
        .then(function() {
            ...
            return airvantage
                .deleteApplications({ selection: { label: getCleaningLabel() } })
